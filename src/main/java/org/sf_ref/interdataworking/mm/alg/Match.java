package org.sf_ref.interdataworking.mm.alg;
...
import org.sf_ref.interdataworking.UntypedGateway;
...
public class Match implements UntypedGateway {
    ...
    public static final int DEBUG_MAX_ITERATIONS = 100;
    ...
    public Match(Model A, Model B, SFConfig config) throws ModelException {
        this.m1 = A;
        this.m2 = B;

        this.FLOW_GRAPH_TYPE = FG_PRODUCT;
        switch (config.fixpointFormula()) {
            case Basic:
                this.formula = FORMULA_FFT;
            case A:
                this.formula = FORMULA_FTF;
                break;
            case B:
                this.formula = FORMULA_TFF;
                break;
            case C:
                this.formula = FORMULA_TTT;
                break;
            default:
                throw new IllegalStateException("Unexpected value: " + config.fixpointFormula());
        }

        Collection<Resource> nodesA = RDFUtil.getNodes(A).values();
        Collection<Resource> nodesB = RDFUtil.getNodes(B).values();

        nodesA.forEach(nA ->
                nodesB.forEach(nB ->
                        {
                            try {
                                initMap.add(new MapPair(nA, nB, config.similarityAlgorithm().apply(nA.getLabel(), nB.getLabel())));
    ...
    public PGNode[] getMatch() throws ModelException {
        //    this.sigma0 = rm;
        ...
        if (initMap.isEmpty())
            ...
            for (Iterator it = initMap.iterator(); it.hasNext(); ) {
...
//     pgarcs = new ArrayList();
	...
	*/
    ...
    static void ICDE02Example() throws Exception {
        ...
        Match sf = new Match(A, B, new SFConfig(StringSimilarity::AllEqual, FixpointFormula.Basic));
    ...
    static void orderedNodesExample(int num) throws Exception {
        ...
        Match sf = new Match(A, B, new SFConfig(StringSimilarity::AllEqual, FixpointFormula.Basic));
        PGNode[] result = sf.getMatch(); // initial mapping is a full cross-product of nodes in A and B
