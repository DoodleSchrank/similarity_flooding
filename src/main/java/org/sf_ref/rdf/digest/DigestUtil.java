...
public class DigestUtil {
  ...
  public static Digest computeDigest(String alg, byte[] b) throws NoSuchAlgorithmException {
    ...
    } catch (org.sf_ref.rdf.digest.DigestException de) {
      // cannot happen
      throw new RuntimeException("Bogus implementation of digest algorithm " + alg + ": " + de.getMessage());
    }
  }

  public static Digest createFromBytes(String alg, byte[] digest) throws org.sf_ref.rdf.digest.DigestException {

    if(Digest.MD5.equals(alg)) {
      if(digest.length != 16)
				throw new org.sf_ref.rdf.digest.DigestException("MD5 digest must be 16 bytes long");
      return new MD5Digest(digest);
    }

    if(Digest.SHA1.equals(alg)) {
      if(digest.length != 20)
				throw new org.sf_ref.rdf.digest.DigestException("SHA-1 digest must be 20 bytes long");
  ...
  public static int getHashCode(Digest d) throws org.sf_ref.rdf.digest.DigestException {
  ...
  public static boolean equal(Digest d1, Digest d2) {
    ...
    } catch (org.sf_ref.rdf.digest.DigestException ex) {}
    return false;
  }
